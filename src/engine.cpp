...
api::SampleRate sampleRateForChartRange(LogViewRange range)
{
    switch (range) {
...
std::string chartRangeLabel(LogViewRange range)
{
    switch (range) {
    case LogViewRange::Hour:
        return "Hour";
...
QDateTime chartWindowStart(LogViewRange range, const QDateTime& end)
{
    switch (range) {
    case LogViewRange::Hour:
...
std::string formatChartTimestamp(qint64 msecs, LogViewRange range)
{
...
std::string formatActionLogEntryValues(const QJsonObject& values)
{
...
std::optional<double> chartLogEntryValue(const QJsonValue& value)
{
    if (value.isBool()) {
